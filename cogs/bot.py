...
class invitebutton(discord.ui.View):
    def __init__(self, select_bot):
        super().__init__()
        self.timeout = None
        self.add_item(discord.ui.Button(label="管理者権限で招待",emoji="👑",style=discord.ButtonStyle.url,
                                         url=f"https://discord.com/api/oauth2/authorize?client_id={select_bot.id}&permissions=8&scope=bot%20applications.commands"))
        self.add_item(discord.ui.Button(label="権限を選択して招待",emoji="🔢",style=discord.ButtonStyle.url,
                                         url=f"https://discord.com/api/oauth2/authorize?client_id={select_bot.id}&permissions=1194000908287&scope=bot%20applications.commands"))
        self.add_item(discord.ui.Button(label="権限無しで招待",emoji="❌",style=discord.ButtonStyle.url,
                                         url=f"https://discord.com/api/oauth2/authorize?client_id={select_bot.id}&scope=bot%20applications.commands"))
